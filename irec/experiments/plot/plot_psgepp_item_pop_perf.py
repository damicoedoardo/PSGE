...
if __name__ == "__main__":
    ...
    # load dfs
    save_name = os.path.join(k_core_dir, "pop_res_df")
    res_df = pd.read_csv(save_name)
    save_name = os.path.join(k_core_dir, "pop_psgepp_res_df")
    ...
    filtered_res_df["Item popularity cluster"] = list(
        map(lambda x: "$< {}$".format(x), filtered_res_df["split_max_count"])
    )
    filtered_sp_res_df["Item popularity cluster"] = list(
    ...
    plt.savefig(
        "{}/egcn_plot/{}.pdf".format(os.environ["HOME"], f"{DATASET}_alpha_item_pop"),
    )
    plt.show()
